...
export class BreakpointsView extends ViewPane {
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this.tree = this.instantiationService.createInstance(
			...
			{
				compressionEnabled: this.configurationService.getValue<boolean>('debug.breakpointsView.showAsTree'),
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('debug.breakpointsView.showAsTree')) {
				const showAsTree = this.configurationService.getValue<boolean>('debug.breakpointsView.showAsTree');
				this.tree.updateOptions({ compressionEnabled: showAsTree });
...
class BreakpointsFolderRenderer implements ICompressibleTreeRenderer<BreakpointsFolderItem, void, IBreakpointsFolderTemplateData> {
	...
	renderTemplate(container: HTMLElement): IBreakpointsFolderTemplateData {
		...
		data.container = dom.append(container, $('.breakpoint-folder'));
		...
		dom.append(data.container, $(ThemeIcon.asCSSSelector(Codicon.file)));
		...
		const badgeContainer = dom.append(data.container, $('.badge-container'));
		data.count = dom.append(badgeContainer, $('span.monaco-count-badge'));
	...
	renderElement(node: ITreeNode<BreakpointsFolderItem, void>, _index: number, data: IBreakpointsFolderTemplateData): void {
		...
		data.name.textContent = resources.basenameOrAuthority(folderItem.uri);
		data.count.textContent = String(folderItem.breakpoints.length);
...
class BreakpointsRenderer implements ICompressibleTreeRenderer<IBreakpoint, void, IBreakpointTemplateData> {
	...
	renderElement(node: ITreeNode<IBreakpoint, void>, index: number, data: IBreakpointTemplateData): void {
		...
		const isChildOfFileNode = node.depth > 0;
		data.context = breakpoint;
		data.breakpoint.classList.toggle('disabled', !this.debugService.getModel().areBreakpointsActivated());

		// For child nodes under a file, show line content; for root nodes show file name
		if (isChildOfFileNode) {
			// Show line content for children of file nodes
			data.name.textContent = localize('loading', "Loading...");
			data.filePath.textContent = '';

			// Async load the line content
			this.textModelService.createModelReference(breakpoint.uri).then(reference => {
				if (data.context !== breakpoint) {
					// Element was recycled, don't update
					reference.dispose();
					return;
				}
	...
	renderCompressedElements(node: ITreeNode<ICompressedTreeNode<IBreakpoint>, void>, index: number, data: IBreakpointTemplateData): void {
		// Breakpoints are not compressible, so we just render the last element
		const breakpoint = node.element.elements[node.element.elements.length - 1];
		data.context = breakpoint;
		this.renderBreakpointData(breakpoint, data);
	}

	private renderBreakpointData(breakpoint: IBreakpoint, data: IBreakpointTemplateData): void {
		data.breakpoint.classList.toggle('disabled', !this.debugService.getModel().areBreakpointsActivated());
